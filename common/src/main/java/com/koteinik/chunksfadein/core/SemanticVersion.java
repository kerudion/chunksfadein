...
/**
 * Parser for a superset of the semantic version format described at
 ...
 * <li>{@code x}, {@code X} or {@code *} for the last {@code <version core>} component with
 * {@code storeX} if not the first
 * <li>Arbitrary {@code <build>} contents
 * </ul>
 */
public class SemanticVersion implements Comparable<SemanticVersion> {
	private static final int COMPONENT_WILDCARD = Integer.MIN_VALUE;
	...
	public SemanticVersion(String version, boolean storeX) throws Exception {
		...
		for (int i = 0; i < componentStrings.length; i++) {
			...
			if (compStr.trim().isEmpty()) {
				throw new Exception("Missing version number component!");
			}
			...
			try {
				...
				if (components[i] < 0) {
					throw new Exception("Negative version number component '" + compStr + "'!");
				}
			} catch (NumberFormatException e) {
				throw new Exception("Could not parse version number component '" + compStr + "'!", e);
			}
		}

		if (storeX && components.length == 1 && components[0] == COMPONENT_WILDCARD) {
			throw new Exception("Versions of form 'x' or 'X' not allowed!");
		}
	...
	public SemanticVersion(int[] components, String prerelease, String build) {
